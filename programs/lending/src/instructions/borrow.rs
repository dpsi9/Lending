...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    match ctx.accounts.mint.to_account_info().key() {
        key if key == user.usdc_address => {
            ...
            total_collateral = price.price as u64 * (user.deposited_sol + accrued_interest);
        }
        _ => {
            let usdc_feed_id = &get_feed_id_from_hex(USDC_USD_FEED_ID)?;
            let price =
                price_update.get_price_no_older_than(&Clock::get()?, MAXIMUM_AGE, usdc_feed_id)?;
            total_collateral = price.price as u64 * user.deposited_usdc;
        }
    }
    ...
    match ctx.accounts.mint.to_account_info().key() {
        ...
        _ => {
            ...
            user.deposited_sol += user_shares;
        }
    }
